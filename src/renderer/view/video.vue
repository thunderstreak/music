<template lang="html">
    <section class="hero-box">
        <VideoHeader @VideoHeaderSelectPlay="VideoHeaderSelectPlay"/>
        <SwitchRouter/>
...
export default {
    ...
    methods:{
        ...
        VideoHeaderSelectPlay(data){
            this.$API.qq.qqMusicMvInfoAPI('u00222le4ox').then(res => {
                console.log(res)
                this.url = res[1].freeflow_url[0]
                if(this.VideoPlayer.paused){
                    this.VideoPlayer.play();
                }
            })
